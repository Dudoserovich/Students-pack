...
protected:
    ...
    void reaction(const Student &s) {
        try {
                ...
                switch (emotion) {
                    ...
                    case EMOTIONS::GOOD:
                        if (s.starStudent) {
                            ...
                            cout << MY_CHILD << s.name << ", самый лучший люблю его! Жаль только, что не отличник :("
                                 << endl;
                        }
                        break;
            ...
            } else throw "Встречен ребёнок, не принадлежащий этому родителю";
...
public:
    Meeting(vector<Parent> parents, vector<Lesson> lessons) : parents(std::move(parents)),
                                                              lessons(std::move(
                                                                      lessons)) {} // в будущем можно there задавать в этом конструкторе, но чтобы было видно, что
